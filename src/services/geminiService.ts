...
- JSON 외 텍스트 금지
        ...
        if (normalizedType === "sns") {
          return `
당신은 동아일보 SNS 포스팅 에디터입니다.
아래 사용자 요청/컨텍스트를 바탕으로 바로 게시 가능한 SNS 문구를 작성하세요.
...
3. 마크다운 문법을 절대 쓰지 마세요. (###, ##, #, **, __, *, -, >, 백틱, 번호 목록 금지)
4. 굵게 표시, 제목 기호, 리스트 기호, 섹션 라벨 없이 일반 텍스트만 출력하세요.
5. 결과는 '한 줄 헤드라인 + 본문 3~5줄 + 해시태그 8~12개' 순서의 자연스러운 일반 텍스트로 작성하세요.
6. '헤드라인:', '본문:', '해시태그:' 같은 표시는 붙이지 마세요.
7. 문장은 자연스럽고 전문적이며, 바로 복사해서 게시할 수 있게 정리하세요.
8. 불필요한 설명, 주석, 안내문 없이 최종 문안만 출력하세요.
          `.trim();
        }
